...
public class PublicationStatusGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all publicationStatuses")
                    .get("/api/publication-statuses")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new publicationStatus")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"id\": \"SAMPLE_TEXT\"" +
                                ", \"publicationStatusName\": \"SAMPLE_TEXT\"" +
                                ", \"publicationStatusCode\": \"SAMPLE_TEXT\"" +
                                ", \"flagActive\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_publicationStatus_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created publicationStatus").get("${new_publicationStatus_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created publicationStatus").delete("${new_publicationStatus_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PublicationStatus entity").exec(scn);
