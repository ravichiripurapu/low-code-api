...
public class PublicationFormGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all publicationForms").get("/api/publication-forms").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new publicationForm")
                        .post("/api/publication-forms")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"fieldId\": \"SAMPLE_TEXT\"" +
                                ", \"fieldClass\": \"SAMPLE_TEXT\"" +
                                ", \"fieldPlaceholder\": \"SAMPLE_TEXT\"" +
                                ", \"fieldOptions\": \"SAMPLE_TEXT\"" +
                                ", \"fieldConfigs\": \"SAMPLE_TEXT\"" +
                                ", \"description\": \"SAMPLE_TEXT\"" +
                                ", \"orderPosition\": 0" +
                                ", \"validationConfigs\": \"SAMPLE_TEXT\"" +
                                ", \"errorMessage\": \"SAMPLE_TEXT\"" +
                                ", \"flagRequired\": null" +
                                ", \"flagActive\": null" +
                                ", \"createUser\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_publicationForm_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created publicationForm").get("${new_publicationForm_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created publicationForm").delete("${new_publicationForm_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PublicationForm entity").exec(scn);
