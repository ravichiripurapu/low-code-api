...
/**
 * Performance test for the PublicationType entity.
 *

 */
public class PublicationTypeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all publicationTypes").get("/api/publication-types").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new publicationType")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"id\": \"SAMPLE_TEXT\"" +
                                ", \"publicationTypeName\": \"SAMPLE_TEXT\"" +
                                ", \"publicationTypeCode\": \"SAMPLE_TEXT\"" +
                                ", \"flagActive\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_publicationType_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created publicationType").get("${new_publicationType_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created publicationType").delete("${new_publicationType_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PublicationType entity").exec(scn);
