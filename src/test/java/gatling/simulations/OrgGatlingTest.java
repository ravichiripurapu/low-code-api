...
public class OrgGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all orgs").get("/api/orgs").headers(headersHttpAuthenticated).check(status().is(200)))
                    ...
                    http("Create new org")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"updatedTimestamp\": \"2020-01-01T00:00:00.000Z\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_org_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created org").get("${new_org_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created org").delete("${new_org_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Org entity").exec(scn);
