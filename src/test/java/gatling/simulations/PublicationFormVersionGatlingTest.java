...
/**
 * Performance test for the PublicationFormVersion entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class PublicationFormVersionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all publicationFormVersions")
                    .get("/api/publication-form-versions")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new publicationFormVersion")
                        .post("/api/publication-form-versions")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"id\": \"SAMPLE_TEXT\"" +
                                ", \"publicationFormVersionName\": \"SAMPLE_TEXT\"" +
                                ", \"publicationFormVersionCode\": \"SAMPLE_TEXT\"" +
                                ", \"flagActive\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_publicationFormVersion_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created publicationFormVersion")
                            .get("${new_publicationFormVersion_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created publicationFormVersion")
                        .delete("${new_publicationFormVersion_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PublicationFormVersion entity").exec(scn);
